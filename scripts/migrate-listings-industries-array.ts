...
/**
 * Migration (Cycle 64): convert listings.industry (TEXT) → listings.industries (TEXT[]).
 *
 *   SUPABASE_ACCESS_TOKEN=... npx tsx scripts/migrate-listings-industries-array.ts            # execute
 *   SUPABASE_ACCESS_TOKEN=... npx tsx scripts/migrate-listings-industries-array.ts --dry-run  # plan only
 *
 ...
 *
 * Legacy `industry` column is left in place for one cycle as a read-only
 * deprecated field; drop scheduled in Cycle 65.
 */
...
`,
  },
  {
    ...
    sql: `
CREATE INDEX IF NOT EXISTS idx_listings_industries_gin
  ON listings USING GIN (industries);
`,
  },
  {
    name: 'mark legacy industry column deprecated',
    sql: `
COMMENT ON COLUMN listings.industry IS
  'DEPRECATED Cycle 64 — use industries TEXT[]. Drop scheduled Cycle 65.';
`,
  },
]
...
async function expectedBackfillCount(): Promise<number> {
  // Pre-migration: count rows with a non-empty legacy industry. Post-migration
  // (or in dry-run after a previous successful run): count rows whose
  // industries[] is still empty but that have a legacy industry to copy.
...
async function verify(): Promise<boolean> {
  ...
  // PostgREST/information_schema reports text[] as data_type "ARRAY", udt_name "_text"
    ...
    await runQuery(`
      SELECT indexname FROM pg_indexes
       WHERE schemaname = 'public'
         ...
         AND indexname = 'idx_listings_industries_gin';
    `)
  )
  if (idxRows.length === 0) {
    console.error('Verification failed: idx_listings_industries_gin missing')
    return false
  }
...
async function run() {
  console.log(`Running listings.industries migration on project: ${SUPABASE_PROJECT_REF}`)
  ...
  console.log('\nVerification passed:')
  console.log('  • listings.industries TEXT[] NOT NULL exists')
  console.log('  • idx_listings_industries_gin in place')
