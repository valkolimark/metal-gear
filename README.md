...
- **Services storefronts (Cycle 70):** sellers and companies can list **Services** with optional SLA and "From $X" pricing. Services use a curated 8-category taxonomy (~41 starter entries — centrifuge rebuilds, pump service, gearbox work, field service, NDT, fabrication, rigging, and more) with a free-text fallback for anything off-taxonomy. Free-tier sellers can configure services and SLA shows publicly; pricing is gated to Pro+ storefronts (lock icon + tooltip publicly; configurable on the edit surface with an upgrade banner). Edit at `/settings/services`; configured services render on `/sellers/[id]` (Services tab + Storefront preview of first 4) and `/companies/[slug]`.
- **Public Team module on company pages (Cycle 70):** companies can opt members into a public **Team** grid on their `/companies/[slug]` storefront. Visibility is **per-member self-service** at `/settings/team-visibility` — admins can request public visibility (sends a notification with a deep link) but cannot toggle it for others. Default privacy: all existing members are private until they opt in. Private member counts are never exposed publicly.
