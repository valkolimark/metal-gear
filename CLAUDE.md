...
- **Rollout status:** consult `docs/navigation-system.md` §11.1 for current rollout state per surface. Update that table every cycle that moves routes. Cycle 72 added the dashboard cluster (`/sos`, `/messages`, `/listings`, `/search`); Cycle 73 added the storefront cluster on the full-bleed shell (`/sellers/[id]`, `/companies/[slug]`, `/profile`, `/profile/[id]`); Cycle 74 added the dashboard secondaries (`/dashboard`, `/radar`, `/credits`, `/notifications`, `/favorites`, `/saved-searches`, `/transactions/*`) on the Dashboard shell and the settings cluster (`/settings/company`, `/settings/services`, `/settings/team-visibility`) on the full-bleed shell. As of Cycle 74, the legacy `(main)` group contains only `/companies/new`, `layout.tsx`, and 8 unmigrated TBD routes (`/boost`, `/checkout`, `/collections`, `/compare`, `/insights`, `/inventory`, `/invite/[token]`, `/schedule`) flagged for Cycle 75 operator decision.
- **Settings uses the full-bleed shell, not Dashboard (Cycle 74 decision):** the settings page provides its own page-level left-rail nav per `/design/settings?variant=desktop-preview`. The global `<AppSidebar>` from the Dashboard shell would create two competing left rails. The full-bleed shell (top bar + mobile bottom nav, no global sidebar) leaves the page free to introduce its own settings nav as a page-level component (Cycle 75) without architectural conflict.
