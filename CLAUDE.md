...
- **Listings industries (Cycle 64):** `listings.industries TEXT[]` (NOT NULL default `'{}'`, GIN-indexed via `idx_listings_industries_gin`). Backfilled from legacy `listings.industry`. Legacy column deprecated, drop scheduled in Cycle 65. Use `MultiIndustryPicker` (`src/components/forms/MultiIndustryPicker.tsx`) for any listing-level industry input. Use `getIndustries()`/`getPrimaryIndustry()` from `src/lib/listings/industries.ts` for any read site that needs to handle both shapes during the deprecation window. Search filter uses `.overlaps('industries', selected)`. Free-text industries are stored as `other:<slug>` and humanised via `industryDisplayLabel()`. Cap of 5 per listing
