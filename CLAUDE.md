...
- **Vision integration (deferred to 61b):** `analyzeEquipmentImages()` will accept a `registryLookup` callback parameter so the vision-analysis layer remains domain-isolated. Architecture grep `grep -rnE "from ['\"]\@?/lib/registry" src/lib/vision-analysis/` must return empty.
