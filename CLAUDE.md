...
- **Route group convention:** `src/app/(main-new-nav)/` hosts routes migrated to the new nav. Currently contains only `/feed`. Cycles 72–75 migrate additional clusters one at a time. Routes still under `src/app/(main)/` continue to use the legacy chrome (`Header` / `DesktopNav` / `MobileNavClient` / `mobile-drawer`).
...
- **Rollout status:** consult `docs/navigation-system.md` §11.1 for current rollout state per surface. Update that table every cycle that moves routes. Cycle 72 added the dashboard cluster (`/sos`, `/messages`, `/listings`, `/search`).
...
- **No new Postgres RPCs for read paths** (Cycle 69 deviation continues). All trust/services/team metrics are computed in TS.
