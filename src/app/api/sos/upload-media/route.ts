...
import { uploadSOSMedia } from '@/lib/media'
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  if (!file || !sosId) {
    return NextResponse.json({ error: 'Missing file or sosId' }, { status: 400 })
  }
  ...
  try {
    ...
    const url = await uploadSOSMedia(buffer, sosId, file.type)
    return NextResponse.json({ url })
  } catch (err) {
    console.error('SOS image upload error:', err)
