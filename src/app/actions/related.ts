...
export async function getRelatedListings(listingId: string, limit = 6) {
  ...
  const { data: source } = await admin
    .from('listings')
    .select('id, category, industries, price_cents, title')
    .eq('id', listingId)
    .single()
  ...
  const { data: candidates } = await admin
    .from('listings')
    .select('id, title, category, industries, price_cents, condition, location_city, location_state, views_count, favorites_count, contact_for_price, seller_id, created_at')
    .eq('status', 'active')
    .eq('has_media', true)
    .neq('id', listingId)
    .limit(100)
  ...
  const scored = candidates.map((c) => {
    ...
    {
      const srcIndustries: string[] = Array.isArray(source.industries) ? source.industries : []
      const candidateIndustries: string[] = Array.isArray(c.industries) ? c.industries : []
