...
interface OnboardingClientProps {
  enabledArchetypes: Archetype[]
}

export default function OnboardingClient({ enabledArchetypes }: OnboardingClientProps) {
      ...
      <div className="flex-1">
        {currentStep === 1 && (
          <StepArchetype
            ...
            enabledArchetypes={enabledArchetypes}
          />
        )}
...
function StepArchetype({
  ...
  const allArchetypes: { id: Archetype; icon: typeof HardHat; name: string; description: string; examples: string; badge?: string }[] = [
    {
      id: 'operator',
      icon: HardHat,
      name: 'Operator',
  ...
  // Cycle 66 — render only enabled archetypes; legacy / disabled types stay in
  // the constants array (preserving type completeness + grandfathered users).
  const archetypes = allArchetypes.filter((a) => enabledArchetypes.includes(a.id))
