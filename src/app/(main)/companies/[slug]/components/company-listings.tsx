...
export function CompanyListings({ listings, totalCount, companyName, companyId }: CompanyListingsProps) {
  return (
    <section>
      <div className="mb-4 flex items-center justify-between">
        <h2 className="font-display text-xl font-semibold">
          Active Listings ({totalCount})
        </h2>
        {totalCount > 12 && (
          <Link
            href={`/search?company_id=${companyId}`}
            className="text-sm text-primary hover:underline"
          >
            View all from {companyName} &rarr;
          </Link>
        )}
      </div>
      {listings.length === 0 ? (
        <div className="rounded-xl border border-border bg-card p-8 text-center">
          ...
          <p className="text-muted-foreground">No active listings from {companyName}.</p>
        </div>
      ) : (
